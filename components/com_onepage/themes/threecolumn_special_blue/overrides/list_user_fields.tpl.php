	   	...
	   	foreach( $rowFields['fields'] as $field) {
			...
			if (((!empty($op_usernameisemail) && ($field['name'] == 'opc_password') ) ) && VM_REGISTRATION_TYPE == 'OPTIONAL_REGISTRATION' ) {
				echo '<div class="formLabel" style="margin-top: 10px;">
						<input type="checkbox" style="width: 10%;" id="register_account" name="register_account" value="1" class="inputbox" onchange="showFields( this.checked, new Array( \'password\', \'password2\') );if( this.checked ) { document.adminForm.remember.value=\'yes\'; } else { document.adminForm.remember.value=\'yes\'; }" autocomplete="off" />
			 ...
			 if (((!empty($op_usernameisemail) && ($field['name'] == 'opc_password') ) ) && VM_REGISTRATION_TYPE != 'NO_REGISTRATION' ) 
			  echo '<input type="hidden" id="register_account" name="register_account" value="1" />';
	   		...
	   		if(( $field['name'] == 'username')  && VM_REGISTRATION_TYPE == 'OPTIONAL_REGISTRATION' ) {
				...
				echo '<div class="formLabel" style="margin-top: 10px;">
						<input type="checkbox" style="width: 10%;" id="register_account" name="register_account" value="1" class="inputbox" onchange="showFields( this.checked, new Array(\'username\', \'password\', \'password2\') );if( this.checked ) { document.adminForm.remember.value=\'yes\'; } else { document.adminForm.remember.value=\'yes\'; }" autocomplete="off" />
						<label for="register_account">'.JText::_('COM_VIRTUEMART_ORDER_REGISTER').'</label>
					</div>
					';
...
';
			...
			*/
	   		echo '">'; 
			if (!empty($field['required']))
			echo JText::_('COM_ONEPAGE_MISSING'); 
			echo '</span>';
